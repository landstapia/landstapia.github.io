...
$(document).ready(function() {
  ...
  var setLanguage = function () {
    ...
    LYRICS_ARRAY = lyrics;
    console.log(LYRICS_ARRAY)
    // Use bounce-in animation when setting language
    $('#lyrics').removeClass('on off bounce-in').addClass('off');
    setTimeout(() => {
      $('#lyrics').html(LYRICS_ARRAY[LYRICS_POS]).removeClass('off').addClass('bounce-in');
    }, 50);
  }
  var next = function () {
    $('#lyrics').removeClass('bounce-in on').addClass('off').promise().done(function() {
      if (LYRICS_POS < LYRICS_ARRAY.length - 1)
        LYRICS_POS++;
      $('#lyrics').html(LYRICS_ARRAY[LYRICS_POS]).removeClass('off').addClass('bounce-in');
    });
  }
